...
function getCategoryTier(categoryScore) {
  ...
  return tiers[category].find(
    (t) => categoryScore >= t.min && categoryScore <= t.max,
  );
}
```
...
function getResultContent(scores) {
  const tier =
    scores.overall < 40 ? "low" : scores.overall < 70 ? "medium" : "high";
...
```javascript
// Lazy load questions
const [visibleQuestions, setVisibleQuestions] = useState(questions.slice(0, 3));
...
function sanitizeForStorage(answers) {
  return Object.fromEntries(
    Object.entries(answers).filter(([key]) => !sensitiveFields.includes(key)),
  );
}
```
