...
export function SliderInput({
  ...
  const scheduleFlush = useCallback((scrollLeft: number) => {
  ...
  const handleMouseUp = () => {
    ...
    if (scrollRef.current) {
      ...
      const closest = Math.round(scrollRef.current.scrollLeft / tickWidth) * tickWidth
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const newScroll = clamp(startScrollLeft.current - dx, 0, scrollRef.current.scrollWidth)
