...
declare global {
  interface Window {
    fbq?: (type: string, event: string, params?: Record<string, unknown>, options?: Record<string, unknown>) => void
  }
}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (step === 1) window.fbq('trackCustom', 'QuizStarted', {}, { eventID: eventId })
    if (step === 10) window.fbq('trackCustom', 'QuizHalfway', {}, { eventID: eventId })
    if (step === 22) window.fbq('trackCustom', 'QuizSalesPage', {}, { eventID: eventId })
  }, [step])
    ...
    <div className="min-h-screen bg-background text-text-primary px-4 pt-2 pb-8 md:pb-12 overflow-x-hidden flex flex-col items-center">
      ...
      <main className="w-full max-w-3xl mx-auto flex-1 flex flex-col items-center justify-start relative overflow-x-hidden min-h-[60vh]">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 8 && <Step08_SocialProofCarousel key="step8" onContinue={() => handleNext()} />}
          ...
          {step === 11 && <Step11_FocusAreas key="step11" onNext={(val: string) => handleNext('regioes_foco', val)} />}
          {step === 12 && <Step12_ActivityLevel key="step12" onNext={(val) => handleNext('nível_atividade', val)} />}
          {step === 13 && <Step13_EnergyLevels key="step13" onNext={(val: string) => handleNext('nivel_energia', val)} />}
          {step === 14 && <Step14_WallOfLove key="step14" onContinue={() => handleNext()} />}
          {step === 15 && <Step15_SleepAndDiet key="step15" onNext={(val: string) => handleNext('sono_e_dieta', val)} />}
          {step === 16 && <Step16_Obstacles key="step16" onNext={(val: string) => handleNext('obstaculos', val)} />}
          {step === 17 && <Step17_Cravings key="step17" onNext={(val: string) => handleNext('vontade_comer', val)} />}
          {step === 18 && <Step18_MainReason key="step18" onNext={(val: string) => handleNext('motivo_entrar_forma', val)} />}
          {step === 19 && <Step19_EnergyBenefit key="step19" onNext={() => handleNext()} />}
          {step === 20 && <Step20_Measurements key="step20" onNext={(val) => handleNext('medidas', val)} />}
          {step === 21 && <Step21_Processing key="step21" onNext={() => handleNext()} />}
          {step === 22 && <Step22_Checkout key="step22" onNext={() => { }} answers={quizData} />}
